...
| D-HOSTING | **ANSWERED 2026-09-07 (owner, via AskUserQuestion)** — drop `output: 'export'` in `next.config.ts`; Vercel runs this app as a normal server app so `src/app/api/**` executes for real production requests, with a real server identity (Firebase Admin SDK, never the client web key) for any server-side Firestore write. Phase `HOSTING-1` builds this; the owner still redeploys/reconfigures Vercel themselves, and still runs any `firebase deploy` for rules separately | no phase may assume this has shipped until `HOSTING-1` is `MERGED_DEVELOP`+`E2E_DEVELOP` and the owner has actually redeployed |
...
| D-PLANS | **ANSWERED 2026-09-07 (owner, via AskUserQuestion)** — the owner's own numbers are authoritative: Basic ₹999, Standard ₹1,800, Premium ₹3,500, Enterprise ₹5,000, replacing BOTH prior conflicting sets (`constants.ts`'s 480/1200/5000 and the payment route's 999/2999/7999/14999). Feature mapping per the owner: Basic = normal company features; Standard = advanced company/landing features; Premium = premium themes + advanced features; Enterprise = full AI access + enterprise features. Phase `PRICING-1` makes this the single source of truth | no plan-price or plan-feature-mapping change without this row |
| D-DOMAIN | should a public job seeker's "personal website" support a real connected custom domain (yourname.com)? | **ANSWERED 2026-09-07 (owner, via AskUserQuestion)** — no; a clean THENIJOBS-hosted URL only (e.g. `thenijobs.com/p/<slug>`), no third-party domain connection, no added hosting cost. Phase `SEEKERSITE-1` builds this scope only |
| D-AI-ACCESS | how does a job seeker's own AI provider connection get paid for, and does any plan bypass it? | **ANSWERED 2026-09-07 (owner)** — ₹50 is a one-time THENIJOBS fee to activate/connect a seeker's own AI API keys (OpenAI/Gemini), distinct from actual provider usage cost (billed by the provider to the user directly) and distinct from `SEEKERPRIVACY-1`'s own ₹50/year public-profile fee — two separate ₹50 products, not one. Enterprise-plan (₹5,000) companies get full AI feature access without this per-connection fee, per `D-PLANS`'s feature mapping. Phase `AI-CONNECT-1` builds this |
