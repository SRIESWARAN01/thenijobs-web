    ...
    <div className="animate-fade-in-up space-y-6 max-w-5xl mx-auto font-outfit text-white">
      ...
      {loading ? (
        ...
        <div className="glass-card rounded-2xl p-12 text-center border border-white/5">
          ...
          <h3 className="text-lg font-semibold text-white mb-2">No businesses match your filters</h3>
          <p className="text-gray-400 text-sm mb-6 max-w-md mx-auto">
            Try adjusting your search or filters to find what you are looking for.
          </p>
          <button 
            onClick={() => { setSearch(''); setSelectedCategory('All'); setSelectedDistrict('All'); setShowVerifiedOnly(false); }}
            className="bg-white/5 border border-white/10 px-5 py-2 rounded-xl text-sm font-medium hover:bg-white/10 transition-all"
          >
            Clear Filters
          </button>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {filtered.map(biz => (
            <div key={biz.id} className="glass-card rounded-2xl p-5 flex flex-col gap-4 border border-white/5 hover:border-violet-500/20 transition-all duration-300">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-1">
                    <div className="min-w-0">
                      ...
                      {biz.isPremium && <span className="text-[9px] font-bold text-violet-400 bg-violet-400/10 px-1.5 py-0.5 rounded border border-violet-400/20 mt-1 inline-block">FEATURED</span>}
                      {biz.isNew && !biz.isPremium && <span className="text-[9px] px-1.5 py-0.5 rounded bg-cyan-500/15 text-cyan-400 border border-cyan-500/20 font-bold inline-block mt-1">NEW</span>}
                    </div>
                  </div>
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-500 pt-2 border-t border-white/5">
              ...
              <div className="flex gap-2 mt-auto">
                ...
                {biz.phone && (
                  <a href={`tel:${biz.phone}`} className="p-2 bg-white/5 border border-white/10 hover:border-violet-500/30 text-gray-400 hover:text-white rounded-xl text-xs font-medium flex items-center gap-1 transition-all" aria-label={`Call ${biz.name}`}>
                    <Phone size={12} />
                  </a>
                )}
                {biz.whatsapp && (
                  <a href={`https://wa.me/${biz.whatsapp.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="p-2 bg-white/5 border border-white/10 hover:border-violet-500/30 text-gray-400 hover:text-white rounded-xl text-xs font-medium flex items-center gap-1 transition-all" aria-label={`WhatsApp ${biz.name}`}>
