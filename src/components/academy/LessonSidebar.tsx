          ...
          return (
            <div key={mod.id} className="space-y-1.5">
              <div className="px-2 py-1 flex items-center justify-between border-b border-white/[0.04]">
                ...
                {!isModuleUnlocked && <Lock size={10} className="text-gray-500" />}
              </div>
              ...
              <div className="space-y-1 pl-1">
                {mod.lessons.map((lesson, lessonIdx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between p-2.5 rounded-xl text-left text-xs transition-all ${isActive ? 'bg-violet-500/20 text-violet-300 border border-violet-500/30' : isLessonUnlocked ? 'hover:bg-white/[0.03] text-gray-400' : 'opacity-40 cursor-not-allowed text-gray-500'}`}
                    >
                      ...
                      {lesson.videoDuration ? (
                        <span className="text-[9px] text-gray-500 shrink-0">{Math.round(lesson.videoDuration / 60)}m</span>
                      ) : null}
                    </button>
                  );
                })}
                ...
                {mod.quizId && (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-2.5 rounded-xl text-left text-xs transition-all ${activeQuizModuleId === mod.id ? 'bg-amber-500/10 text-amber-300 border border-amber-500/20' : allLessonsCompleted ? 'text-amber-400/80 hover:bg-amber-500/5' : 'opacity-40 cursor-not-allowed text-gray-500'}`}
                  >
                    <div className="flex items-center gap-2 truncate">
                      {quizPassed ? (
                        <CheckCircle2 size={13} className="text-emerald-400 shrink-0" />
