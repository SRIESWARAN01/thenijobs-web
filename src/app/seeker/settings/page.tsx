...
export default function SeekerSettingsPage() {
  const { user } = useAuth();
  const { data: remoteSettings, loading } = useDocument<any>('seekerSettings', user?.uid);

  const [prefs, setPrefs] = useState(initialPrefs);
  const [privacy, setPrivacy] = useState({
    viewResumeAfterApply: true,
    showPhoneInSearch: false,
    whatsappContactOptIn: true,
  });
  ...
  useEffect(() => {
    if (remoteSettings) {
      if (remoteSettings.prefs) setPrefs(remoteSettings.prefs);
      if (remoteSettings.privacy) setPrivacy(remoteSettings.privacy);
    }
  }, [remoteSettings]);

  const handleSave = async (updatedPrefs = prefs, updatedPrivacy = privacy) => {
    if (!user?.uid) return;
    setAutoSaveStatus('saving');
    try {
      await upsertDocument('seekerSettings', user.uid, {
        userId: user.uid,
        prefs: updatedPrefs,
      ...
      console.error('Failed to save seeker settings:', err);
  ...
  const toggle = (event: EventKey, channel: Channel) => {
    const updated = {
      ...
      [event]: {
        ...prefs[event],
        [channel]: !prefs[event][channel],
      },
    };
    setPrefs(updated);
    handleSave(updated, privacy);
  };
      ...
      <div className="rounded-2xl border border-emerald-500/20 bg-gradient-to-r from-emerald-500/10 to-cyan-500/5 p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              type="button"
              onClick={() => handleSave(prefs, privacy)}
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            <div className="glass-card rounded-2xl p-5">
              ...
              <div className="mt-4 space-y-3">
                {[
                  ['Employers can view resume after application', privacy.viewResumeAfterApply, 'viewResumeAfterApply'],
                  ['Talent search can show phone number', privacy.showPhoneInSearch, 'showPhoneInSearch'],
                  ['WhatsApp direct contact opt-in', privacy.whatsappContactOptIn, 'whatsappContactOptIn'],
                ].map(([label, checked, key]) => (
                  <div key={String(label)} className="flex items-center justify-between gap-4">
                    <span className="text-sm text-gray-400">{label}</span>
                    <Toggle checked={Boolean(checked)} onClick={() => togglePrivacy(key as any)} />
                  </div>
                ))}
              </div>
            </div>
