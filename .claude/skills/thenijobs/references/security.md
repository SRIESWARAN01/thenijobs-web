...
|---|---|---|---|
| S-1 | **Catch-all rules nullify every specific rule**: `firestore.rules` L394–398 `read: true / write: isAuthenticated()`; `storage.rules` L114–118 `read: true / write: isAuthenticated() && <10MB` | rule text; memory 2026-09-03 REST probe (read permitted). **FIXED IN TREE by RULES-1 (`6a4c0a0`, 2026-09-04):** both catch-alls removed; 42 emulator probes pass, 23 flip on the old rules. **Production unchanged until the owner deploys** the rules and quotes the Playground after-check | **P0 in production** until deployed |
| S-2 | Duplicate blocks: `platformSettings` L248 (admin read) vs L383 (public read); `portfolioSites` L289 (owner-bound create) vs L375 (any-auth create) — permissive wins | rule text. **FIXED IN TREE by RULES-1** (strict copies kept) | P0 in production until deployed |
| S-3 | `subscriptionPlan` (companies) and `aiCredits`/`aiCreditsUsed`, `companyId`, `isEmployer`, `canPostJobs` (users) are outside the `*ModerationUnchanged()` lists | `firestore.rules` L56–73; writers `payment/verify/route.ts` L181/L200, `creditService.ts:72`, `firestoreService.ts` L620–625. **FIXED IN TREE by RULES-1** (guards extended; probes P3, P4, X6a deny) | P0 in production until deployed |
...
| S-10 | Admin AI settings store plaintext provider `apiKey` in `platformSettings/aiConfig`, which S-2 makes world-readable | `src/app/admin/ai-settings/page.tsx:105`, `aiConfigService.ts` L44–53. **Exposure closed in tree by RULES-1** (admin-only read; probe P8 denies); the plaintext storage itself remains | P1 (plaintext at rest) once deployed; owner still checks whether a key was ever saved |
| S-11 | Storage: any authenticated user may write under another user's `/portfolio/{uid}/**` and `/uploads/{uid}/**` | `storage.rules` L103, L110. **FIXED IN TREE by RULES-1** (owner-only; probe P9a denies) | P1 in production until deployed |
| S-12 | `counters/*` read/write by any authenticated user; `seoAnalytics` write by any authenticated user (**fixed in tree by RULES-1: admin-only**); `conversations` + `messages` create without participant check; `activityLogs` create by anyone authenticated; `leads` create unauthenticated with no limit | `firestore.rules` L314–316, L326, L343, L242, L188 (line numbers at `5b61111`) | P2 — remaining items are their own phase |
| S-13 | `register-business/page.tsx:128` creates a company with `isActive: true` — works only through S-1; the S-1 fix breaks this flow unless fixed together | source. **FIXED IN TREE by RULES-1** (`isActive: false`; probe P10a allows the new shape, P10b denies the old one) | closed with S-1 |
