...
export interface CompanyJobDetailManagerProps {
  jobId: string;
  /** The company that owns this job -- used only to fill in the share/preview modals' company
   *  fields (name, logo, category, district, verification, rating). */
  companyId: string;
  /** Where the "Jobs" breadcrumb link and post-write navigation (archive/delete) point, e.g.
   *  '/employer/jobs' or '/admin/businesses/<id>/jobs'. */
  basePath: string;
  /** Breadcrumb trail before "Jobs" -- e.g. [{label:'Employer', href:'/employer/dashboard'}] or
   *  [{label:'Admin', href:'/admin/dashboard'}, {label:'Businesses', href:'/admin/businesses'}]. */
  breadcrumbs: Crumb[];
...
/**
 * The job detail page -- header, quick-update actions, overview/applications/performance/history
 * tabs -- shared by the employer's own job detail page
 * (src/app/employer/jobs/[id]/EmployerJobDetailClient.tsx) and the admin company jobs manager
 * (src/app/admin/businesses/[id]/jobs/[jobId]/**). Persistence writes directly to Firestore (same
 * as the original employer-only page did); only company resolution, navigation targets and the
 * breadcrumb trail are parameterised.
 */
export default function CompanyJobDetailManager({ jobId, companyId, basePath, breadcrumbs, onWrite }: CompanyJobDetailManagerProps) {
  const router = useRouter();
  const toast = useToast();

  const { data: job, loading: jobLoading } = useDocument<any>('jobs', jobId);
  const { data: company } = useDocument<any>('companies', companyId);
  ...
  const handleStatusChange = async (newStatus: JobStatus) => {
    ...
    try {
      ...
      onWrite?.(status === 'pending' ? 'Resumed job posting' : status === 'paused' ? 'Paused job posting' : 'Closed job posting', `Job ${jobId} status set to ${status}`);
  ...
  const handleArchive = async () => {
    ...
    try {
      ...
      onWrite?.('Archived job posting', `Job ${jobId} archived`);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      onWrite?.('Deleted job posting', `Job ${jobId} permanently deleted`);
      router.push(basePath);
    } catch (e) { console.error(e); }
    finally { setActionLoading(null); }
  };
  ...
  if (!job) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
        ...
        <Link href={basePath} className="text-sm font-semibold text-blue-600 hover:text-blue-800">← Back to Jobs</Link>
      </div>
    );
  }
  ...
  return (
    <PageShell className="max-w-5xl">
      <PageHeader
        title={job.title}
        breadcrumbs={[...breadcrumbs, { label: 'Jobs', href: basePath }, { label: job.title }]}
        actions={
          <>
