...
export interface PortfolioSiteEditorSaveFields {
  sections: PortfolioSection[];
  theme: PortfolioTheme;
  branding: PortfolioSite['branding'];
  seo: PortfolioSite['seo'];
}

export interface PortfolioSiteEditorProps {
  /** The site to edit, as currently persisted. */
  initialSite: PortfolioSite;
  /** Subscription plan slug that gates which sections can be added (getPortfolioSectionsForPlan). */
  planSlug: string;
  /** Where the top-left back arrow returns to. */
  backHref: string;
  /** Called with the fields to persist. The caller decides how (and, for an admin caller, also
   *  logs the action via logActivity — this component has no opinion on that). */
...
/**
 * The section/theme/branding/SEO editor + live preview, shared by the employer's own website
 * editor (src/app/employer/website/editor/page.tsx) and the admin website manager
...
export default function PortfolioSiteEditor({ initialSite, planSlug, backHref, onSave, onPublishToggle }: PortfolioSiteEditorProps) {
  ...
  const removeSection = useCallback((sectionId: string) => {
    setSite(prev => ({ ...prev, sections: prev.sections.filter(s => s.id !== sectionId) }));
    setEditor(prev => ({ ...prev, isDirty: true }));
  }, []);
    ...
    <div className="flex flex-col h-[calc(100vh-72px)]" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-white border-b border-gray-100 flex-shrink-0">
        <div className="flex items-center gap-3">
          <Link href={backHref} className="p-2 rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50">
            <ArrowLeft size={14} />
          </Link>
          <div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div>
                      <label htmlFor="portfolio-site-editor-body-font" className="text-[10px] text-gray-500 block mb-1">Body Font</label>
                      <select id="portfolio-site-editor-body-font" value={site.theme.fontFamily} onChange={e => updateSiteField('theme.fontFamily', e.target.value)}
                        className="w-full px-2 py-2 border border-gray-200 rounded-lg text-base sm:text-xs">
                        {FONT_OPTIONS.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}
                      </select>
                    </div>
                    <div>
                      <label htmlFor="portfolio-site-editor-heading-font" className="text-[10px] text-gray-500 block mb-1">Heading Font</label>
                      <select id="portfolio-site-editor-heading-font" value={site.theme.headingFont} onChange={e => updateSiteField('theme.headingFont', e.target.value)}
                        className="w-full px-2 py-2 border border-gray-200 rounded-lg text-base sm:text-xs">
                        {FONT_OPTIONS.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}
                      </select>
                    </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div>
                      <label htmlFor="portfolio-site-editor-border-radius" className="text-[10px] text-gray-500 block mb-1">Border Radius</label>
                      <select id="portfolio-site-editor-border-radius" value={site.theme.borderRadius} onChange={e => updateSiteField('theme.borderRadius', e.target.value)}
                        className="w-full px-2 py-2 border border-gray-200 rounded-lg text-base sm:text-xs">
                    ...
                    <div>
                      <label htmlFor="portfolio-site-editor-animation" className="text-[10px] text-gray-500 block mb-1">Animation</label>
                      <select id="portfolio-site-editor-animation" value={site.theme.animation} onChange={e => updateSiteField('theme.animation', e.target.value)}
                        className="w-full px-2 py-2 border border-gray-200 rounded-lg text-base sm:text-xs">
            ...
            {activeTab === 'branding' && (
              <div className="space-y-4">
                {[
                  ...
                  <div key={field.key}>
                    <label htmlFor={`portfolio-site-editor-branding-${field.key}`} className="text-[10px] font-semibold text-gray-500 uppercase block mb-1">{field.label}</label>
                    <input id={`portfolio-site-editor-branding-${field.key}`} type={field.type} value={(site.branding as any)[field.key] || ''}
            ...
            {activeTab === 'seo' && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input id="portfolio-site-editor-keywords-comma-separated" type="text" value={site.seo.keywords?.join(', ') || ''} onChange={e => updateSiteField('seo.keywords', e.target.value.split(',').map((k: string) => k.trim()).filter(Boolean))}
                    className="w-full px-3 py-2 border border-gray-200 rounded-xl text-base sm:text-xs" />
                </div>
