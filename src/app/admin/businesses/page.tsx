...
export default function BusinessesPage() {
  ...
  const handleVerificationLevelChange = async (bizId: string, level: string) => {
    setActionLoading(bizId);
    try {
      await updateDocument('companies', bizId, {
      ...
      alert(`Verification level updated to ${level.toUpperCase()}`);
    } catch (err) {
      console.error('Verification level change error:', err);
          ...
          <div className="space-y-6 text-gray-300 font-outfit max-h-[75vh] overflow-y-auto pr-2">
            ...
            <div className="border-t border-white/[0.08] pt-5 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-gray-400">Verification Level</label>
                  <div className="flex gap-2">
                    <select
                      value={selectedBiz.verificationLevel || 'free'}
                      onChange={(e) => handleVerificationLevelChange(selectedBiz.id, e.target.value)}
                      className="w-full rounded-xl bg-white/[0.04] border border-white/[0.08] px-3 py-2 text-sm text-white focus:outline-none focus:border-violet-500/50"
                    >
                      <option value="free">Free (No Badge)</option>
                      <option value="standard">Standard (Blue Tick)</option>
                      <option value="premium">Premium (Gold Tick)</option>
                      <option value="elite">Elite (VIP Crown)</option>
                    </select>
                  </div>
                </div>
