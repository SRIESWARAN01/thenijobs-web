...
import { Star, MessageSquare, Loader2, Award, Calendar, Reply } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-fade-in-up font-outfit text-white">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400 mt-1">Monitor ratings and reply to user feedback on your business profile</p>
      </div>
                ...
                filtered.map((review) => (
                  <div key={review.id} className="glass-card rounded-2xl p-5 hover:border-white/15 transition-all">
                    <div className="flex items-start gap-4">
                      {/* Avatar */}
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-500/20 to-teal-500/20 flex items-center justify-center flex-shrink-0">
                      ...
                      <div className="flex-1 min-w-0 space-y-2">
                        ...
                        {/* Reply box */}
                        {review.replyText ? (
                          <div className="bg-[#0e0e22] p-3 rounded-xl border border-emerald-500/10 space-y-1">
                          ...
                          replyReviewId !== review.id && (
                            <button
                              ...
                              className="text-[10px] font-bold text-emerald-400 hover:text-emerald-300 flex items-center gap-1 pt-1"
                            >
                        ...
                        {replyReviewId === review.id && (
                          <div className="pt-2 space-y-2">
                            <textarea
                              ...
                              className="w-full bg-[#111124] border border-white/10 rounded-lg p-2 text-xs text-white placeholder:text-gray-500 focus:border-emerald-500/40 focus:outline-none resize-none"
                            />
                            <div className="flex gap-2">
                              <button
                                onClick={() => handlePostReply(review.id)}
                                disabled={actionLoading === review.id || !replyInput.trim()}
                                className="px-3 py-1.5 rounded-lg bg-emerald-600 text-white text-[10px] font-bold hover:bg-emerald-500 transition-colors disabled:opacity-50 flex items-center gap-1"
                              >
