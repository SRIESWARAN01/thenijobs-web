...
export default function ServiceReviewsPage() {
  ...
  // 1. Fetch service provider's company profile
  ...
  if (!companyId && !companyLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center font-outfit text-white">
        ...
        <h2 className="text-lg font-semibold text-white">No Service Profile</h2>
        <p className="text-sm text-gray-400 mt-2 max-w-sm">Please register your profile first to view and respond to customer reviews.</p>
  ...
  return (
    <div className="space-y-6 animate-fade-in-up font-outfit text-white">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400 mt-1">Monitor ratings and reply to user feedback on your service profile</p>
      </div>
                ...
                filtered.map((review) => (
                  <div key={review.id} className="glass-card rounded-2xl p-5 hover:border-white/15 transition-all">
                    <div className="flex items-start gap-4">
                      {/* Avatar */}
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-rose-500/20 to-pink-500/20 flex items-center justify-center flex-shrink-0">
                      ...
                      <div className="flex-1 min-w-0 space-y-2">
                        ...
                        {/* Reply box */}
                        {review.replyText ? (
                          <div className="bg-[#0e0e22] p-3 rounded-xl border border-rose-500/10 space-y-1">
                          ...
                          replyReviewId !== review.id && (
                            <button
                              ...
                              className="text-[10px] font-bold text-rose-400 hover:text-rose-300 flex items-center gap-1 pt-1"
                            >
                        ...
                        {replyReviewId === review.id && (
                          <div className="pt-2 space-y-2">
                            <textarea
                              ...
                              className="w-full bg-[#111124] border border-white/10 rounded-lg p-2 text-xs text-white placeholder:text-gray-600 focus:border-rose-500/40 focus:outline-none resize-none"
                            />
                            <div className="flex gap-2">
                              <button
                                onClick={() => handlePostReply(review.id)}
                                disabled={actionLoading === review.id || !replyInput.trim()}
                                className="px-3 py-1.5 rounded-lg bg-rose-600 text-white text-[10px] font-bold hover:bg-rose-500 transition-colors disabled:opacity-50 flex items-center gap-1"
                              >
