...
const statusColors: Record<string, string> = {
  ...
  paused: 'bg-amber-100 text-amber-400',
  ended: 'bg-gray-500/10 text-gray-400',
  draft: 'bg-blue-100 text-cyan-400' };

const typeColors: Record<string, string> = {
  Banner: 'bg-violet-500/10 text-violet-400',
  Sponsored: 'bg-blue-100 text-cyan-400',
  Featured: 'bg-amber-100 text-amber-400' };
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 font-outfit">Advertisement Management</h1>
          <p className="text-sm text-slate-500 mt-1">Manage banner ads, sponsored listings, and campaigns</p>
        </div>
      </div>
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-white/[0.02] transition-colors">
                      <td className="px-5 py-3.5">
                        <p className="text-sm font-medium text-gray-900">{c.title || 'Untitled Campaign'}</p>
                            ...
                            <>
                              {adStatus === 'active' ? (
                                <button
                                  ...
                                  className="p-1.5 rounded-lg bg-amber-100 text-amber-400 hover:bg-amber-500/20"
