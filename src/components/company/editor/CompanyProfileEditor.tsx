...
export interface CompanyProfileEditorProps {
  /** The company doc as currently persisted. Undefined only for an employer who has not saved a
   *  company yet -- the admin caller always resolves a real doc before rendering this component. */
  initialCompany: (Partial<CompanyProfileEditorSaveFields> & {
    id?: string;
    slug?: string;
  ...
  description: string;
  breadcrumbs: Crumb[];
  /** True for the admin caller -- swaps a few pronoun-bearing strings ("your" -> "this company's"). */
  viewerIsAdmin?: boolean;
  /** Called with the fields to persist. The caller decides how (create vs update) and, for an
   *  admin caller, also logs the action via logActivity -- this component has no opinion on that. */
  onSave: (fields: CompanyProfileEditorSaveFields) => Promise<void>;
}

/**
 * The company profile form (branding, contact, location, socials, products, services, portfolio,
 * founder, reviews, sections), shared by the employer's own profile page
 * (src/app/employer/company-profile/page.tsx) and the admin profile manager
 * (src/app/admin/businesses/[id]/profile/page.tsx). Persistence is delegated via onSave so each
 * caller controls its own write path (and, for admin, its own audit log).
 */
export default function CompanyProfileEditor({ initialCompany, title, description, breadcrumbs, viewerIsAdmin, onSave }: CompanyProfileEditorProps) {
  ...
  const handleSave = async () => {
    ...
    if (!company.address || !company.district) {
      toast.error('Please fill in Address and District.');
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <PageShell className="pb-24 sm:pb-12">
      ...
      {initialCompany?.verificationStatus === 'pending' && (
        <div className="rounded-3xl p-4 sm:p-5 border border-amber-200 bg-amber-50/90 flex items-start gap-3 shadow-xs">
          ...
          <div className="text-xs sm:text-sm">
            ...
            <p className="text-amber-800 mt-0.5 text-xs leading-relaxed">
              {viewerIsAdmin
                ? "This company's profile has been submitted and is awaiting verification. You can continue updating its details here."
      ...
      {initialCompany?.verificationStatus === 'rejected' && (
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0 text-xs sm:text-sm">
              ...
              <p className="text-red-800 mt-0.5 text-xs">
                {viewerIsAdmin
                  ? "This company's profile requires changes before it can go live."
                  : 'Your business profile requires changes before it can go live.'}
              </p>
              {initialCompany?.rejectionReason && (
                <div className="mt-2 p-3 rounded-2xl bg-white border border-red-200">
                  ...
                  <span className="text-xs text-red-800">{initialCompany.rejectionReason}</span>
                </div>
              )}
              <p className="text-[11px] text-gray-500 mt-2">
                {viewerIsAdmin
                  ? 'Update the profile below and save — changes are recorded immediately.'
      ...
      {initialCompany?.verificationStatus === 'verified' && (
        <div className="rounded-2xl p-3.5 border border-emerald-200 bg-emerald-50/80 flex items-center gap-2.5 shadow-xs">
          ...
          <p className="text-xs text-emerald-800 font-bold">
            {viewerIsAdmin ? 'This company is verified and live on THENIJOBS.' : 'Your business is verified and live on THENIJOBS.'}
          </p>
        </div>
      )}
      ...
      <div className="flex items-center gap-1.5 border-b border-slate-200 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 no-scrollbar">
        {[
          ...
          { key: 'products', label: 'Products Catalogue', icon: Package, locked: !hasFeaturePermission(initialCompany?.subscriptionPlan, 'productsListing') },
          { key: 'services', label: 'Services Directory', icon: Wrench, locked: !hasFeaturePermission(initialCompany?.subscriptionPlan, 'servicesListing') },
          { key: 'portfolio', label: 'Portfolio & Projects', icon: FolderGit2, locked: !hasFeaturePermission(initialCompany?.subscriptionPlan, 'portfolioProjects') },
          { key: 'founder', label: 'Founder Profile', icon: User, locked: !hasFeaturePermission(initialCompany?.subscriptionPlan, 'founderProfile') },
      ...
      {activeTab === 'products' && (
        <div className="bg-white rounded-3xl p-4 sm:p-6 border border-gray-200 shadow-xs">
          <CompanyProductsManager
            products={company.products || []}
            planSlug={initialCompany?.subscriptionPlan || 'free'}
            companyName={company.name || 'Company'}
            companySlug={initialCompany?.slug || ''}
      ...
      {activeTab === 'reviews' && (
        <div className="bg-white rounded-3xl p-4 sm:p-6 border border-gray-200 shadow-xs">
          <CompanyReviewsManager companyId={initialCompany?.id || ''} />
        </div>
      )}
          ...
          <div className="xl:col-span-2 space-y-5 sm:space-y-6">
            ...
            <div className="bg-white rounded-3xl p-4 sm:p-6 border border-gray-200 shadow-xs space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4">
                <div>
                  <label htmlFor="company-profile-editor-primary-calling-phone" className="text-xs text-gray-700 font-bold block mb-1">Primary Calling Phone *</label>
                  <div className="relative">
                    <Phone size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" />
                    <input id="company-profile-editor-primary-calling-phone"
                ...
                <div>
                  <label htmlFor="company-profile-editor-official-email" className="text-xs text-gray-700 font-bold block mb-1">Official Email *</label>
                  <div className="relative">
                    <Mail size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" />
                    <input id="company-profile-editor-official-email"
                ...
                <div>
                  <label htmlFor="company-profile-editor-whatsapp-number" className="text-xs text-gray-700 font-bold block mb-1">WhatsApp Number</label>
                  <div className="relative">
                    <MessageCircle size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-emerald-500" />
                    <input id="company-profile-editor-whatsapp-number"
                ...
                <div>
                  <label htmlFor="company-profile-editor-company-website-url" className="text-xs text-gray-700 font-bold block mb-1">Company Website URL</label>
                  <div className="relative">
                    <Globe size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" />
                    <input id="company-profile-editor-company-website-url"
            ...
            <div className="bg-white rounded-3xl p-4 sm:p-6 border border-gray-200 shadow-xs space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4">
                <div className="sm:col-span-2">
                  ...
                  <input id="company-profile-editor-full-door-address"
                    type="text"
                ...
                <div>
                  <label htmlFor="company-profile-editor-district-region" className="text-xs text-gray-700 font-bold block mb-1">District / Region *</label>
                  <select id="company-profile-editor-district-region"
                ...
                <div>
                  <label htmlFor="company-profile-editor-established-year" className="text-xs text-gray-700 font-bold block mb-1">Established Year</label>
                  <input id="company-profile-editor-established-year"
                    type="text"
            ...
            <div className="bg-white rounded-3xl p-4 sm:p-6 border border-gray-200 shadow-xs space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4">
                <div>
                  <label htmlFor="company-profile-editor-facebook" className="text-xs text-gray-700 font-bold block mb-1 flex items-center gap-1.5">
                    <Link2 size={13} className="text-blue-600" /> Facebook
                  </label>
                  <input id="company-profile-editor-facebook"
                    type="url"
                ...
                <div>
                  <label htmlFor="company-profile-editor-instagram" className="text-xs text-gray-700 font-bold block mb-1 flex items-center gap-1.5">
                    <Heart size={13} className="text-pink-600" /> Instagram
                  </label>
                  <input id="company-profile-editor-instagram"
                    type="url"
                ...
                <div>
                  <label htmlFor="company-profile-editor-linkedin" className="text-xs text-gray-700 font-bold block mb-1 flex items-center gap-1.5">
                    <LinkedinIcon size={13} className="text-blue-700" /> LinkedIn
                  </label>
                  <input id="company-profile-editor-linkedin"
                    type="url"
                ...
                <div>
                  <label htmlFor="company-profile-editor-youtube" className="text-xs text-gray-700 font-bold block mb-1 flex items-center gap-1.5">
                    <Play size={13} className="text-red-600" /> YouTube
                  </label>
                  <input id="company-profile-editor-youtube"
                    type="url"
      ...
      <DeviceLivePreviewModal
        ...
        publicUrl={initialCompany?.slug ? `/company/${initialCompany.slug}` : '/company/digital-theni-solutions'}
      >
