...
exports.verifyAdPayment = exports.createAdOrder = exports.verifyRazorpayPayment = exports.createRazorpayOrder = exports.processSubscriptionAutomation = exports.validateSubscriptionAccess = void 0;
...
exports.createAdOrder = (0, https_1.onCall)({ region: config_1.REGION, enforceAppCheck: false }, async (request) => {
    const uid = requireUid(request);
    ...
    const placement = getString(request.data?.placement);
    const companyId = getString(request.data?.companyId);
    ...
    if (!keyId || !keySecret || keyId === 'mock_key_id') {
        ...
        firebase_functions_1.logger.info(`[MOCK AD] Creating order for user ${uid}, plan ${planId}`);
        await config_1.db.doc(`razorpayOrders/${mockOrderId}`).set({
            orderId: mockOrderId,
            userId: uid,
            planId,
    ...
    try {
        ...
        firebase_functions_1.logger.error('Error creating Ad Razorpay order:', err);
        throw new https_1.HttpsError('internal', err?.message || 'Error creating Razorpay order');
    }
});
exports.verifyAdPayment = (0, https_1.onCall)({ region: config_1.REGION, enforceAppCheck: false }, async (request) => {
    const uid = requireUid(request);
    ...
    if (!paymentId || !orderId || (!signature && !orderId.startsWith('order_mock_ad_'))) {
        throw new https_1.HttpsError('invalid-argument', 'Missing signature parameters.');
    }
    ...
    try {
        const result = await config_1.db.runTransaction(async (transaction) => {
            ...
            let collectionName = '';
            if (placement === 'job') {
                const ref = config_1.db.doc(`jobs/${targetId}`);
                const snap = await transaction.get(ref);
                targetTitle = snap.data()?.title || 'Job Post';
                collectionName = 'jobs';
            }
            else if (placement === 'product') {
                const ref = config_1.db.doc(`products/${targetId}`);
                const snap = await transaction.get(ref);
                targetTitle = snap.data()?.name || 'Product';
                collectionName = 'products';
            }
            else {
                const ref = config_1.db.doc(`services/${targetId}`);
                const snap = await transaction.get(ref);
            ...
            if (collectionName) {
                const ref = config_1.db.doc(`${collectionName}/${targetId}`);
                transaction.update(ref, {
                    isPromoted: true,
                    promotedUntil: firestore_1.Timestamp.fromDate(endDate),
            ...
            const transactionRef = config_1.db.collection('adTransactions').doc();
            ...
            const adRef = config_1.db.collection('advertisements').doc();
        ...
        firebase_functions_1.logger.error('Error verifying Ad payment:', err);
        throw new https_1.HttpsError('internal', err?.message || 'Error verifying Ad payment');
    }
});
