...
import { getCourse, getModules, getLessons, getCourseQuizzes, updateCourse, addModule, updateModule, deleteModule, addLesson, updateLesson, deleteLesson, createQuiz, updateQuiz } from '@/lib/firebase/lmsService';
import { useAuth } from '@/hooks/useAuth';
import { COURSE_CATEGORIES } from '@/lib/types/lms';
import type { CourseDifficulty, QuizQuestion, Course, CourseModule, Lesson, Quiz, LessonType } from '@/lib/types/lms';
...
const STEPS = ['Basic Info', 'Modules & Lessons', 'Review & Save'];
...
export default function EditCoursePage() {
  ...
  // Original IDs to track deletions
  const [originalModules, setOriginalModules] = useState<CourseModule[]>([]);
  const [originalLessons, setOriginalLessons] = useState<Record<string, Lesson[]>>({});
  ...
  useEffect(() => {
    ...
    const loadCourseData = async () => {
      try {
        ...
        setOriginalModules(dbMods);

        const quizzes = await getCourseQuizzes(courseId);
        setOriginalQuizzes(quizzes);

        const modForms: ModuleForm[] = [];
        const lessonsMap: Record<string, Lesson[]> = {};

        for (const mod of dbMods) {
          const dbLessons = await getLessons(courseId, mod.id);
          lessonsMap[mod.id] = dbLessons;

          const associatedQuiz = quizzes.find(q => q.moduleId === mod.id);
          ...
          modForms.push({
            ...
            quiz: associatedQuiz ? {
              id: associatedQuiz.id,
              title: associatedQuiz.title,
              passingScore: associatedQuiz.passingScore,
              allowRetake: associatedQuiz.allowRetake,
              maxAttempts: associatedQuiz.maxAttempts,
              questions: associatedQuiz.questions,
            } : null,
          });
        }

        setOriginalLessons(lessonsMap);
        setModules(modForms.length > 0 ? modForms : [{ title: '', description: '', lessons: [{ title: '', type: 'video', youtubeVideoId: '', videoDuration: 0, content: '', isPreview: false }], quiz: null }]);
      } catch (err) {
  ...
  const handleSave = async (publish: boolean) => {
    ...
    try {
      ...
      // Track items kept to delete orphans
      const keptModuleIds: string[] = [];
      const keptLessonIds: string[] = [];
      const keptQuizIds: string[] = [];
      ...
      for (let mIdx = 0; mIdx < modules.length; mIdx++) {
        ...
        if (moduleId) {
          ...
          // Add module
          moduleId = await addModule(courseId, {
            title: mod.title.trim(),
            description: mod.description.trim(),
            order: mIdx + 1,
          });
        ...
        for (let lIdx = 0; lIdx < mod.lessons.length; lIdx++) {
          ...
          if (lesson.id) {
            await updateLesson(courseId, moduleId, lesson.id, lessonData);
            keptLessonIds.push(lesson.id);
          } else {
            const newLessonId = await addLesson(courseId, moduleId, lessonData);
            keptLessonIds.push(newLessonId);
          }
        }
        ...
        if (mod.quiz) {
          ...
          if (mod.quiz.id) {
            await updateQuiz(mod.quiz.id, quizData);
            keptQuizIds.push(mod.quiz.id);
          } else {
            const newQuizId = await createQuiz(quizData);
            keptQuizIds.push(newQuizId);
          }
        }
      }

      // Cleanup Deleted Modules
      for (const origMod of originalModules) {
        if (!keptModuleIds.includes(origMod.id)) {
          // Delete its lessons first
          const origModLessons = originalLessons[origMod.id] || [];
          for (const origL of origModLessons) {
            await deleteLesson(courseId, origMod.id, origL.id);
          }
          await deleteModule(courseId, origMod.id);
        } else {
          // Cleanup Deleted Lessons inside kept module
          const origModLessons = originalLessons[origMod.id] || [];
          for (const origL of origModLessons) {
            if (!keptLessonIds.includes(origL.id)) {
              await deleteLesson(courseId, origMod.id, origL.id);
            }
          }
        }
      ...
      // Cleanup Deleted Quizzes
      for (const origQuiz of originalQuizzes) {
        if (!keptQuizIds.includes(origQuiz.id)) {
          // Simple delete is update doc with status deleted or similar, but for quizzes we can delete
          await deleteDoc(doc(db, 'quizzes', origQuiz.id));
        }
      }
      ...
      alert('Failed to update course: ' + (err.message || err));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in-up max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-white font-outfit">Edit Course</h1>
        ...
        <div className="glass-card rounded-2xl p-6 space-y-4">
          <div>
            ...
            <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g., Tally ERP Masterclass"
              className="w-full bg-white/[0.03] border border-white/[0.08] px-4 py-2.5 text-sm rounded-xl text-white placeholder:text-gray-500 focus:border-violet-500/30 outline-none" />
          </div>
          <div>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)} rows={4} placeholder="Describe what this course covers..."
              className="w-full bg-white/[0.03] border border-white/[0.08] px-4 py-2.5 text-sm rounded-xl text-white placeholder:text-gray-500 focus:border-violet-500/30 outline-none resize-none" />
          </div>
              ...
              {mod.quiz ? (
                <div className="bg-amber-500/5 border border-amber-500/15 rounded-xl p-3 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    }} placeholder="Passing score %" className="bg-white/[0.03] border border-white/[0.06] px-3 py-1.5 text-xs rounded-lg text-white outline-none" />
      ...
      {step === 2 && (
        <div className="glass-card rounded-2xl p-6 space-y-4">
          <h2 className="text-lg font-bold text-white">Review Your Changes</h2>
          ...
          {step < STEPS.length - 1 ? (
            ...
            <>
              <button onClick={() => handleSave(false)} disabled={saving}
                ...
                {saving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} Keep Draft
              </button>
              <button onClick={() => handleSave(true)} disabled={saving}
                ...
                {saving ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />} Publish Changes
              </button>
            </>
          )}
        </div>
