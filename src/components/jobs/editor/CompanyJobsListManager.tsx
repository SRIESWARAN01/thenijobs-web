...
export interface CompanyJobsListManagerProps {
  /** The company whose job postings this list shows and edits. */
  companyId: string;
  /** For the share-job modal's "companyName" field. */
  companyName: string;
  /** Base path a job title/row links into, e.g. '/employer/jobs' or '/admin/businesses/<id>/jobs'. */
  basePath: string;
  /** href for the "Post New Job" button. Omit to hide it entirely -- posting a brand-new job is
   *  its own large, separate flow (/employer/post-job, 849 lines) not covered by this component. */
  postNewHref?: string;
  /** Called after every write this component makes. The caller decides how (or whether) to log
   *  it -- an admin caller logs via logActivity(), the employer caller omits this entirely. */
  onWrite?: (action: string, details: string) => void;
}

/**
 * The job list -- filter tabs, KPI cards, table/grid of postings with status-change/duplicate/
 * close/delete actions -- shared by the employer's own jobs page (src/app/employer/jobs/page.tsx)
 * and the admin company jobs manager (src/app/admin/businesses/[id]/jobs/page.tsx). Persistence
 * writes directly to Firestore (same as the original employer-only page did); only company
 * resolution and the post-new-job entry point are parameterised.
 */
export default function CompanyJobsListManager({ companyId, companyName, basePath, postNewHref, onWrite }: CompanyJobsListManagerProps) {
  ...
  const [view, setView] = useViewMode('company-jobs-list', 'table');
  ...
  const handleToggleStatus = async (id: string, currentStatus: JobStatus) => {
    ...
    try {
      ...
      onWrite?.(next === 'pending' ? 'Resumed job posting' : 'Paused job posting', `Job ${id} status set to ${next}`);
  ...
  const handleCloseJob = async (id: string) => {
    ...
    try {
      ...
      onWrite?.('Closed job posting', `Job ${id} closed`);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      onWrite?.('Deleted job posting', `Job ${id} permanently deleted`);
  ...
  const handleDuplicate = async (job: JobDoc) => {
    ...
    try {
      ...
      onWrite?.('Duplicated job posting', `"${job.title}" duplicated as a new draft`);
    ...
    <div className="space-y-6 font-outfit text-gray-900 pb-20 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-0.5">Manage, track applications, and monitor job postings</p>
        </div>
        {postNewHref && (
          <Link
            href={postNewHref}
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
            ...
            <div className="relative sm:w-72">
              ...
              <input
                type="text"
                aria-label="Search job titles" placeholder="Search job titles..."
                value={search}
          ...
          <DataTable
            ...
            emptyDescription={postNewHref ? 'Switch tabs, or post a new vacancy to get started.' : 'Switch tabs to see other job postings.'}
            emptyAction={postNewHref ? (
              <Link href={postNewHref}>
            ...
            rowActions={job => {
              ...
              const items: ActionItem[] = [
                { label: 'View analytics', icon: Eye, href: `${basePath}/${job.id}` },
              ];
